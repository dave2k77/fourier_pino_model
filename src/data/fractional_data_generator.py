#!/usr/bin/env python3
"""
Fractional Data Generator for FractionalPINO Experiments
Generate synthetic data for fractional PDE problems
"""
...
def generate_fractional_wave_data(
    ...
    inputs = torch.zeros(batch_size, time_steps, 3)
    outputs = u_analytical.view(batch_size, time_steps, -1)
    
    for i in range(batch_size):
        for j, t_val in enumerate(t):
            inputs[i, j, 0] = X_flat[0]  # x coordinate
            inputs[i, j, 1] = Y_flat[0]  # y coordinate
            inputs[i, j, 2] = t_val      # time
