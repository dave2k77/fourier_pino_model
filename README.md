...
We used a number of helper functions to transform the input data into a format that can easily be used by the PINO model. These helper functions can be found in the `preprocess` file found here: [preprocess](https://github.com/dave2k77/fourier_pino_model/blob/master/src/preprocess.py)
